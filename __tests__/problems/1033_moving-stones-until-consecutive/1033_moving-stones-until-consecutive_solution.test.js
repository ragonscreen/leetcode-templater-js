...
beforeAll(async () => {
        SOURCE = JSON.parse(
                await readFile(
                        './__tests__/problems/1033_moving-stones-until-consecutive/1033_moving-stones-until-consecutive_response.txt',
                        'utf-8',
                ),
        );
});

describe('1033_moving-stones-until-consecutive', () => {
        test('solution template', () => {
                ...
                expect(solution).toMatchSnapshot('1033_moving-stones-until-consecutive');
        });
});
