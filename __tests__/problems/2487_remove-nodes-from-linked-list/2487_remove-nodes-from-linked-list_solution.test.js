...
beforeAll(async () => {
        SOURCE = JSON.parse(
                await readFile(
                        './__tests__/problems/2487_remove-nodes-from-linked-list/2487_remove-nodes-from-linked-list_response.txt',
                        'utf-8',
                ),
        );
});

describe('2487_remove-nodes-from-linked-list', () => {
        test('solution template', () => {
                ...
                expect(solution).toMatchSnapshot('2487_remove-nodes-from-linked-list');
        });
});
