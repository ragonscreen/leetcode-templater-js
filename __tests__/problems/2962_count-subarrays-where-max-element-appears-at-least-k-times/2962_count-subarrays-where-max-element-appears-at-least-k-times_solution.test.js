...
beforeAll(async () => {
        SOURCE = JSON.parse(
                await readFile(
                        './__tests__/problems/2962_count-subarrays-where-max-element-appears-at-least-k-times/2962_count-subarrays-where-max-element-appears-at-least-k-times_response.txt',
                        'utf-8',
                ),
        );
});

describe('2962_count-subarrays-where-max-element-appears-at-least-k-times', () => {
        test('solution template', () => {
                ...
                expect(solution).toMatchSnapshot('2962_count-subarrays-where-max-element-appears-at-least-k-times');
        });
});
