...
beforeAll(async () => {
        SOURCE = JSON.parse(
                await readFile(
                        './__tests__/problems/1665_minimum-initial-energy-to-finish-tasks/1665_minimum-initial-energy-to-finish-tasks_response.txt',
                        'utf-8',
                ),
        );
});

describe('1665_minimum-initial-energy-to-finish-tasks', () => {
        test('solution template', () => {
                ...
                expect(solution).toMatchSnapshot('1665_minimum-initial-energy-to-finish-tasks');
        });
});
