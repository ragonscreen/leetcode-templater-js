...
beforeAll(async () => {
        SOURCE = JSON.parse(
                await readFile(
                        './__tests__/problems/1100_find-k-length-substrings-with-no-repeated-characters/1100_find-k-length-substrings-with-no-repeated-characters_response.txt',
                        'utf-8',
                ),
        );
});

describe('1100_find-k-length-substrings-with-no-repeated-characters', () => {
        test('solution template', () => {
                ...
                expect(solution).toMatchSnapshot('1100_find-k-length-substrings-with-no-repeated-characters');
        });
});
