...
beforeAll(async () => {
        SOURCE = JSON.parse(
                await readFile(
                        './__tests__/problems/0453_minimum-moves-to-equal-array-elements/0453_minimum-moves-to-equal-array-elements_response.txt',
                        'utf-8',
                ),
        );
});

describe('0453_minimum-moves-to-equal-array-elements', () => {
        test('solution template', () => {
                ...
                expect(solution).toMatchSnapshot('0453_minimum-moves-to-equal-array-elements');
        });
});
