...
beforeAll(async () => {
        SOURCE = JSON.parse(
                await readFile(
                        './__tests__/problems/2211_count-collisions-on-a-road/2211_count-collisions-on-a-road_response.txt',
                        'utf-8',
                ),
        );
});

describe('2211_count-collisions-on-a-road', () => {
        test('solution template', () => {
                ...
                expect(solution).toMatchSnapshot('2211_count-collisions-on-a-road');
        });
});
