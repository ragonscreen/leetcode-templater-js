...
beforeAll(async () => {
        SOURCE = JSON.parse(
                await readFile(
                        './__tests__/problems/0720_longest-word-in-dictionary/0720_longest-word-in-dictionary_response.txt',
                        'utf-8',
                ),
        );
});

describe('0720_longest-word-in-dictionary', () => {
        test('solution template', () => {
                ...
                expect(solution).toMatchSnapshot('0720_longest-word-in-dictionary');
        });
});
