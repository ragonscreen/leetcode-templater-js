...
beforeAll(async () => {
        SOURCE = JSON.parse(
                await readFile(
                        './__tests__/problems/1047_remove-all-adjacent-duplicates-in-string/1047_remove-all-adjacent-duplicates-in-string_response.txt',
                        'utf-8',
                ),
        );
});

describe('1047_remove-all-adjacent-duplicates-in-string', () => {
        test('solution template', () => {
                ...
                expect(solution).toMatchSnapshot('1047_remove-all-adjacent-duplicates-in-string');
        });
});
