...
beforeAll(async () => {
        SOURCE = JSON.parse(
                await readFile(
                        './__tests__/problems/0300_longest-increasing-subsequence/0300_longest-increasing-subsequence_response.txt',
                        'utf-8',
                ),
        );
});

describe('0300_longest-increasing-subsequence', () => {
        test('solution template', () => {
                ...
                expect(solution).toMatchSnapshot('0300_longest-increasing-subsequence');
        });
});
