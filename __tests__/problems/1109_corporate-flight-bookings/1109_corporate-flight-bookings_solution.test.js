...
beforeAll(async () => {
        SOURCE = JSON.parse(
                await readFile(
                        './__tests__/problems/1109_corporate-flight-bookings/1109_corporate-flight-bookings_response.txt',
                        'utf-8',
                ),
        );
});

describe('1109_corporate-flight-bookings', () => {
        test('solution template', () => {
                ...
                expect(solution).toMatchSnapshot('1109_corporate-flight-bookings');
        });
});
