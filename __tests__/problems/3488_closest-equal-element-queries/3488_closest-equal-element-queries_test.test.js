...
beforeAll(async () => {
        SOURCE = JSON.parse(
                await readFile(
                        './__tests__/problems/3488_closest-equal-element-queries/3488_closest-equal-element-queries_response.txt',
                        'utf-8',
                ),
        );
});

describe('3488_closest-equal-element-queries', () => {
        test('test template', () => {
                ...
                expect(test).toMatchSnapshot('3488_closest-equal-element-queries');
        });
});
