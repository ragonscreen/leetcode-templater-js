...
beforeAll(async () => {
        SOURCE = JSON.parse(
                await readFile(
                        './__tests__/problems/3653_xor-after-range-multiplication-queries-i/3653_xor-after-range-multiplication-queries-i_response.txt',
                        'utf-8',
                ),
        );
});

describe('3653_xor-after-range-multiplication-queries-i', () => {
        test('solution template', () => {
                ...
                expect(solution).toMatchSnapshot('3653_xor-after-range-multiplication-queries-i');
        });
});
