...
beforeAll(async () => {
        SOURCE = JSON.parse(
                await readFile(
                        './__tests__/problems/2197_replace-non-coprime-numbers-in-array/2197_replace-non-coprime-numbers-in-array_response.txt',
                        'utf-8',
                ),
        );
});

describe('2197_replace-non-coprime-numbers-in-array', () => {
        test('solution template', () => {
                ...
                expect(solution).toMatchSnapshot('2197_replace-non-coprime-numbers-in-array');
        });
});
