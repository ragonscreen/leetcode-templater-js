...
beforeAll(async () => {
        SOURCE = JSON.parse(
                await readFile(
                        './__tests__/problems/1962_remove-stones-to-minimize-the-total/1962_remove-stones-to-minimize-the-total_response.txt',
                        'utf-8',
                ),
        );
});

describe('1962_remove-stones-to-minimize-the-total', () => {
        test('solution template', () => {
                ...
                expect(solution).toMatchSnapshot('1962_remove-stones-to-minimize-the-total');
        });
});
