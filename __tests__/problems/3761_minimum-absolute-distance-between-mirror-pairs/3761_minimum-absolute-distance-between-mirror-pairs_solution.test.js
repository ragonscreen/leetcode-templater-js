...
beforeAll(async () => {
        SOURCE = JSON.parse(
                await readFile(
                        './__tests__/problems/3761_minimum-absolute-distance-between-mirror-pairs/3761_minimum-absolute-distance-between-mirror-pairs_response.txt',
                        'utf-8',
                ),
        );
});

describe('3761_minimum-absolute-distance-between-mirror-pairs', () => {
        test('solution template', () => {
                ...
                expect(solution).toMatchSnapshot('3761_minimum-absolute-distance-between-mirror-pairs');
        });
});
