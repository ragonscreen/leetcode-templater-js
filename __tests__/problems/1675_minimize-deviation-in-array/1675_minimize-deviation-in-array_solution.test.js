...
beforeAll(async () => {
        SOURCE = JSON.parse(
                await readFile(
                        './__tests__/problems/1675_minimize-deviation-in-array/1675_minimize-deviation-in-array_response.txt',
                        'utf-8',
                ),
        );
});

describe('1675_minimize-deviation-in-array', () => {
        test('solution template', () => {
                ...
                expect(solution).toMatchSnapshot('1675_minimize-deviation-in-array');
        });
});
