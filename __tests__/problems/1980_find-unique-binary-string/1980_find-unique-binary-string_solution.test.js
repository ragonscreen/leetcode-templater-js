...
beforeAll(async () => {
        SOURCE = JSON.parse(
                await readFile(
                        './__tests__/problems/1980_find-unique-binary-string/1980_find-unique-binary-string_response.txt',
                        'utf-8',
                ),
        );
});

describe('1980_find-unique-binary-string', () => {
        test('solution template', () => {
                ...
                expect(solution).toMatchSnapshot('1980_find-unique-binary-string');
        });
});
