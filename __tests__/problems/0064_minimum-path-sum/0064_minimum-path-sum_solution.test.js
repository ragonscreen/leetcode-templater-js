...
beforeAll(async () => {
        SOURCE = JSON.parse(
                await readFile(
                        './__tests__/problems/0064_minimum-path-sum/0064_minimum-path-sum_response.txt',
                        'utf-8',
                ),
        );
});

describe('0064_minimum-path-sum', () => {
        test('solution template', () => {
                ...
                expect(solution).toMatchSnapshot('0064_minimum-path-sum');
        });
});
