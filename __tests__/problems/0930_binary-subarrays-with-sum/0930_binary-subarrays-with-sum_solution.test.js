...
beforeAll(async () => {
        SOURCE = JSON.parse(
                await readFile(
                        './__tests__/problems/0930_binary-subarrays-with-sum/0930_binary-subarrays-with-sum_response.txt',
                        'utf-8',
                ),
        );
});

describe('0930_binary-subarrays-with-sum', () => {
        test('solution template', () => {
                ...
                expect(solution).toMatchSnapshot('0930_binary-subarrays-with-sum');
        });
});
