...
beforeAll(async () => {
        SOURCE = JSON.parse(
                await readFile(
                        './__tests__/problems/1151_minimum-swaps-to-group-all-1s-together/1151_minimum-swaps-to-group-all-1s-together_response.txt',
                        'utf-8',
                ),
        );
});

describe('1151_minimum-swaps-to-group-all-1s-together', () => {
        test('solution template', () => {
                ...
                expect(solution).toMatchSnapshot('1151_minimum-swaps-to-group-all-1s-together');
        });
});
