...
beforeAll(async () => {
        SOURCE = JSON.parse(
                await readFile(
                        './__tests__/problems/1391_check-if-there-is-a-valid-path-in-a-grid/1391_check-if-there-is-a-valid-path-in-a-grid_response.txt',
                        'utf-8',
                ),
        );
});

describe('1391_check-if-there-is-a-valid-path-in-a-grid', () => {
        test('solution template', () => {
                ...
                expect(solution).toMatchSnapshot('1391_check-if-there-is-a-valid-path-in-a-grid');
        });
});
