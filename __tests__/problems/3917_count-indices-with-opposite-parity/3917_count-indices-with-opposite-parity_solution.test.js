...
beforeAll(async () => {
        SOURCE = JSON.parse(
                await readFile(
                        './__tests__/problems/3917_count-indices-with-opposite-parity/3917_count-indices-with-opposite-parity_response.txt',
                        'utf-8',
                ),
        );
});

describe('3917_count-indices-with-opposite-parity', () => {
        test('solution template', () => {
                ...
                expect(solution).toMatchSnapshot('3917_count-indices-with-opposite-parity');
        });
});
