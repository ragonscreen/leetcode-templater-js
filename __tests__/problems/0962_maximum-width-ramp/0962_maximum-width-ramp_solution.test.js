...
beforeAll(async () => {
        SOURCE = JSON.parse(
                await readFile(
                        './__tests__/problems/0962_maximum-width-ramp/0962_maximum-width-ramp_response.txt',
                        'utf-8',
                ),
        );
});

describe('0962_maximum-width-ramp', () => {
        test('solution template', () => {
                ...
                expect(solution).toMatchSnapshot('0962_maximum-width-ramp');
        });
});
