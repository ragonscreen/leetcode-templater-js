...
beforeAll(async () => {
        SOURCE = JSON.parse(
                await readFile(
                        './__tests__/problems/2553_separate-the-digits-in-an-array/2553_separate-the-digits-in-an-array_response.txt',
                        'utf-8',
                ),
        );
});

describe('2553_separate-the-digits-in-an-array', () => {
        test('solution template', () => {
                ...
                expect(solution).toMatchSnapshot('2553_separate-the-digits-in-an-array');
        });
});
