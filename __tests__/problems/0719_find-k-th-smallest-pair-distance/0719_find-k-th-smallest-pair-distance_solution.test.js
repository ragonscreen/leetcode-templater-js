...
beforeAll(async () => {
        SOURCE = JSON.parse(
                await readFile(
                        './__tests__/problems/0719_find-k-th-smallest-pair-distance/0719_find-k-th-smallest-pair-distance_response.txt',
                        'utf-8',
                ),
        );
});

describe('0719_find-k-th-smallest-pair-distance', () => {
        test('solution template', () => {
                ...
                expect(solution).toMatchSnapshot('0719_find-k-th-smallest-pair-distance');
        });
});
