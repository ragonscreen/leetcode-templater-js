...
beforeAll(async () => {
        SOURCE = JSON.parse(
                await readFile(
                        './__tests__/problems/1578_minimum-time-to-make-rope-colorful/1578_minimum-time-to-make-rope-colorful_response.txt',
                        'utf-8',
                ),
        );
});

describe('1578_minimum-time-to-make-rope-colorful', () => {
        test('solution template', () => {
                ...
                expect(solution).toMatchSnapshot('1578_minimum-time-to-make-rope-colorful');
        });
});
