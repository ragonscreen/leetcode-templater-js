...
beforeAll(async () => {
        SOURCE = JSON.parse(
                await readFile(
                        './__tests__/problems/0209_minimum-size-subarray-sum/0209_minimum-size-subarray-sum_response.txt',
                        'utf-8',
                ),
        );
});

describe('0209_minimum-size-subarray-sum', () => {
        test('solution template', () => {
                ...
                expect(solution).toMatchSnapshot('0209_minimum-size-subarray-sum');
        });
});
