...
beforeAll(async () => {
        SOURCE = JSON.parse(
                await readFile(
                        './__tests__/problems/2183_count-array-pairs-divisible-by-k/2183_count-array-pairs-divisible-by-k_response.txt',
                        'utf-8',
                ),
        );
});

describe('2183_count-array-pairs-divisible-by-k', () => {
        test('solution template', () => {
                ...
                expect(solution).toMatchSnapshot('2183_count-array-pairs-divisible-by-k');
        });
});
