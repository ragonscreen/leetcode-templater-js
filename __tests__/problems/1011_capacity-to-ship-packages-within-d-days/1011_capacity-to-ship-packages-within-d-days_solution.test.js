...
beforeAll(async () => {
        SOURCE = JSON.parse(
                await readFile(
                        './__tests__/problems/1011_capacity-to-ship-packages-within-d-days/1011_capacity-to-ship-packages-within-d-days_response.txt',
                        'utf-8',
                ),
        );
});

describe('1011_capacity-to-ship-packages-within-d-days', () => {
        test('solution template', () => {
                ...
                expect(solution).toMatchSnapshot('1011_capacity-to-ship-packages-within-d-days');
        });
});
