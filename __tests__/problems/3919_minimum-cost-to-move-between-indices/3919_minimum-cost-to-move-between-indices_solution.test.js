...
beforeAll(async () => {
        SOURCE = JSON.parse(
                await readFile(
                        './__tests__/problems/3919_minimum-cost-to-move-between-indices/3919_minimum-cost-to-move-between-indices_response.txt',
                        'utf-8',
                ),
        );
});

describe('3919_minimum-cost-to-move-between-indices', () => {
        test('solution template', () => {
                ...
                expect(solution).toMatchSnapshot('3919_minimum-cost-to-move-between-indices');
        });
});
