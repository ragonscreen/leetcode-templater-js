...
beforeAll(async () => {
        SOURCE = JSON.parse(
                await readFile(
                        './__tests__/problems/2523_closest-prime-numbers-in-range/2523_closest-prime-numbers-in-range_response.txt',
                        'utf-8',
                ),
        );
});

describe('2523_closest-prime-numbers-in-range', () => {
        test('solution template', () => {
                ...
                expect(solution).toMatchSnapshot('2523_closest-prime-numbers-in-range');
        });
});
