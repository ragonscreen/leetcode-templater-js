...
beforeAll(async () => {
        SOURCE = JSON.parse(
                await readFile(
                        './__tests__/problems/3931_check-adjacent-digit-differences/3931_check-adjacent-digit-differences_response.txt',
                        'utf-8',
                ),
        );
});

describe('3931_check-adjacent-digit-differences', () => {
        test('solution template', () => {
                ...
                expect(solution).toMatchSnapshot('3931_check-adjacent-digit-differences');
        });
});
