...
beforeAll(async () => {
        SOURCE = JSON.parse(
                await readFile(
                        './__tests__/problems/0778_swim-in-rising-water/0778_swim-in-rising-water_response.txt',
                        'utf-8',
                ),
        );
});

describe('0778_swim-in-rising-water', () => {
        test('solution template', () => {
                ...
                expect(solution).toMatchSnapshot('0778_swim-in-rising-water');
        });
});
