...
beforeAll(async () => {
        SOURCE = JSON.parse(
                await readFile(
                        './__tests__/problems/0423_reconstruct-original-digits-from-english/0423_reconstruct-original-digits-from-english_response.txt',
                        'utf-8',
                ),
        );
});

describe('0423_reconstruct-original-digits-from-english', () => {
        test('solution template', () => {
                ...
                expect(solution).toMatchSnapshot('0423_reconstruct-original-digits-from-english');
        });
});
