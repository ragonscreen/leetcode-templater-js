...
beforeAll(async () => {
        SOURCE = JSON.parse(
                await readFile(
                        './__tests__/problems/0693_binary-number-with-alternating-bits/0693_binary-number-with-alternating-bits_response.txt',
                        'utf-8',
                ),
        );
});

describe('0693_binary-number-with-alternating-bits', () => {
        test('solution template', () => {
                ...
                expect(solution).toMatchSnapshot('0693_binary-number-with-alternating-bits');
        });
});
