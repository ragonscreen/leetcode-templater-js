...
beforeAll(async () => {
        SOURCE = JSON.parse(
                await readFile(
                        './__tests__/problems/0303_range-sum-query-immutable/0303_range-sum-query-immutable_response.txt',
                        'utf-8',
                ),
        );
});

describe('0303_range-sum-query-immutable', () => {
        test('solution template', () => {
                ...
                expect(solution).toMatchSnapshot('0303_range-sum-query-immutable');
        });
});
