...
beforeAll(async () => {
        SOURCE = JSON.parse(
                await readFile(
                        './__tests__/problems/2139_minimum-moves-to-reach-target-score/2139_minimum-moves-to-reach-target-score_response.txt',
                        'utf-8',
                ),
        );
});

describe('2139_minimum-moves-to-reach-target-score', () => {
        test('solution template', () => {
                ...
                expect(solution).toMatchSnapshot('2139_minimum-moves-to-reach-target-score');
        });
});
