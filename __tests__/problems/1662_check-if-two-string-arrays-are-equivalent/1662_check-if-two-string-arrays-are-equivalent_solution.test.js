...
beforeAll(async () => {
        SOURCE = JSON.parse(
                await readFile(
                        './__tests__/problems/1662_check-if-two-string-arrays-are-equivalent/1662_check-if-two-string-arrays-are-equivalent_response.txt',
                        'utf-8',
                ),
        );
});

describe('1662_check-if-two-string-arrays-are-equivalent', () => {
        test('solution template', () => {
                ...
                expect(solution).toMatchSnapshot('1662_check-if-two-string-arrays-are-equivalent');
        });
});
