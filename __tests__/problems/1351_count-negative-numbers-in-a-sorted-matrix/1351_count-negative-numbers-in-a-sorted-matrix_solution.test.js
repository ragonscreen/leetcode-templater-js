...
beforeAll(async () => {
        SOURCE = JSON.parse(
                await readFile(
                        './__tests__/problems/1351_count-negative-numbers-in-a-sorted-matrix/1351_count-negative-numbers-in-a-sorted-matrix_response.txt',
                        'utf-8',
                ),
        );
});

describe('1351_count-negative-numbers-in-a-sorted-matrix', () => {
        test('solution template', () => {
                ...
                expect(solution).toMatchSnapshot('1351_count-negative-numbers-in-a-sorted-matrix');
        });
});
