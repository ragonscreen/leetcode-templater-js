...
beforeAll(async () => {
        SOURCE = JSON.parse(
                await readFile(
                        './__tests__/problems/0026_remove-duplicates-from-sorted-array/0026_remove-duplicates-from-sorted-array_response.txt',
                        'utf-8',
                ),
        );
});

describe('0026_remove-duplicates-from-sorted-array', () => {
        test('solution template', () => {
                ...
                expect(solution).toMatchSnapshot('0026_remove-duplicates-from-sorted-array');
        });
});
