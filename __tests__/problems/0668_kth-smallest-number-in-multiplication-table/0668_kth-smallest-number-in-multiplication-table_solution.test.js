...
beforeAll(async () => {
        SOURCE = JSON.parse(
                await readFile(
                        './__tests__/problems/0668_kth-smallest-number-in-multiplication-table/0668_kth-smallest-number-in-multiplication-table_response.txt',
                        'utf-8',
                ),
        );
});

describe('0668_kth-smallest-number-in-multiplication-table', () => {
        test('solution template', () => {
                ...
                expect(solution).toMatchSnapshot('0668_kth-smallest-number-in-multiplication-table');
        });
});
