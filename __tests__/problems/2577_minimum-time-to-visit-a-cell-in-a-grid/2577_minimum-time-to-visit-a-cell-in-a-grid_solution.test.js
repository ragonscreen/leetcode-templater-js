...
beforeAll(async () => {
        SOURCE = JSON.parse(
                await readFile(
                        './__tests__/problems/2577_minimum-time-to-visit-a-cell-in-a-grid/2577_minimum-time-to-visit-a-cell-in-a-grid_response.txt',
                        'utf-8',
                ),
        );
});

describe('2577_minimum-time-to-visit-a-cell-in-a-grid', () => {
        test('solution template', () => {
                ...
                expect(solution).toMatchSnapshot('2577_minimum-time-to-visit-a-cell-in-a-grid');
        });
});
