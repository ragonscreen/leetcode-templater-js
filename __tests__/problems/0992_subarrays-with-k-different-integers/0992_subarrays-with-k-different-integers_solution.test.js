...
beforeAll(async () => {
        SOURCE = JSON.parse(
                await readFile(
                        './__tests__/problems/0992_subarrays-with-k-different-integers/0992_subarrays-with-k-different-integers_response.txt',
                        'utf-8',
                ),
        );
});

describe('0992_subarrays-with-k-different-integers', () => {
        test('solution template', () => {
                ...
                expect(solution).toMatchSnapshot('0992_subarrays-with-k-different-integers');
        });
});
