...
beforeAll(async () => {
        SOURCE = JSON.parse(
                await readFile(
                        './__tests__/problems/2751_robot-collisions/2751_robot-collisions_response.txt',
                        'utf-8',
                ),
        );
});

describe('2751_robot-collisions', () => {
        test('solution template', () => {
                ...
                expect(solution).toMatchSnapshot('2751_robot-collisions');
        });
});
