...
beforeAll(async () => {
        SOURCE = JSON.parse(
                await readFile(
                        './__tests__/problems/1944_number-of-visible-people-in-a-queue/1944_number-of-visible-people-in-a-queue_response.txt',
                        'utf-8',
                ),
        );
});

describe('1944_number-of-visible-people-in-a-queue', () => {
        test('solution template', () => {
                ...
                expect(solution).toMatchSnapshot('1944_number-of-visible-people-in-a-queue');
        });
});
