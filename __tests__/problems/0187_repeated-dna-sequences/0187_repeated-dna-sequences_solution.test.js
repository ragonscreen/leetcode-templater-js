...
beforeAll(async () => {
        SOURCE = JSON.parse(
                await readFile(
                        './__tests__/problems/0187_repeated-dna-sequences/0187_repeated-dna-sequences_response.txt',
                        'utf-8',
                ),
        );
});

describe('0187_repeated-dna-sequences', () => {
        test('solution template', () => {
                ...
                expect(solution).toMatchSnapshot('0187_repeated-dna-sequences');
        });
});
