...
beforeAll(async () => {
        SOURCE = JSON.parse(
                await readFile(
                        './__tests__/problems/0438_find-all-anagrams-in-a-string/0438_find-all-anagrams-in-a-string_response.txt',
                        'utf-8',
                ),
        );
});

describe('0438_find-all-anagrams-in-a-string', () => {
        test('solution template', () => {
                ...
                expect(solution).toMatchSnapshot('0438_find-all-anagrams-in-a-string');
        });
});
