...
beforeAll(async () => {
        SOURCE = JSON.parse(
                await readFile(
                        './__tests__/problems/3746_minimum-string-length-after-balanced-removals/3746_minimum-string-length-after-balanced-removals_response.txt',
                        'utf-8',
                ),
        );
});

describe('3746_minimum-string-length-after-balanced-removals', () => {
        test('solution template', () => {
                ...
                expect(solution).toMatchSnapshot('3746_minimum-string-length-after-balanced-removals');
        });
});
