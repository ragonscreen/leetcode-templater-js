...
beforeAll(async () => {
        SOURCE = JSON.parse(
                await readFile(
                        './__tests__/problems/1784_check-if-binary-string-has-at-most-one-segment-of-ones/1784_check-if-binary-string-has-at-most-one-segment-of-ones_response.txt',
                        'utf-8',
                ),
        );
});

describe('1784_check-if-binary-string-has-at-most-one-segment-of-ones', () => {
        test('solution template', () => {
                ...
                expect(solution).toMatchSnapshot('1784_check-if-binary-string-has-at-most-one-segment-of-ones');
        });
});
