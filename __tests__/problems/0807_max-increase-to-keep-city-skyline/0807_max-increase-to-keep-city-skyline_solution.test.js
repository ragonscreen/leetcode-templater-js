...
beforeAll(async () => {
        SOURCE = JSON.parse(
                await readFile(
                        './__tests__/problems/0807_max-increase-to-keep-city-skyline/0807_max-increase-to-keep-city-skyline_response.txt',
                        'utf-8',
                ),
        );
});

describe('0807_max-increase-to-keep-city-skyline', () => {
        test('solution template', () => {
                ...
                expect(solution).toMatchSnapshot('0807_max-increase-to-keep-city-skyline');
        });
});
