...
beforeAll(async () => {
        SOURCE = JSON.parse(
                await readFile(
                        './__tests__/problems/1208_get-equal-substrings-within-budget/1208_get-equal-substrings-within-budget_response.txt',
                        'utf-8',
                ),
        );
});

describe('1208_get-equal-substrings-within-budget', () => {
        test('solution template', () => {
                ...
                expect(solution).toMatchSnapshot('1208_get-equal-substrings-within-budget');
        });
});
