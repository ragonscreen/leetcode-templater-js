...
beforeAll(async () => {
        SOURCE = JSON.parse(
                await readFile(
                        './__tests__/problems/2906_construct-product-matrix/2906_construct-product-matrix_response.txt',
                        'utf-8',
                ),
        );
});

describe('2906_construct-product-matrix', () => {
        test('solution template', () => {
                ...
                expect(solution).toMatchSnapshot('2906_construct-product-matrix');
        });
});
