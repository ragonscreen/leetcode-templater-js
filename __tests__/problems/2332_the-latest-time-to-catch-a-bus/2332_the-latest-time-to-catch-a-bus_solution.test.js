...
beforeAll(async () => {
        SOURCE = JSON.parse(
                await readFile(
                        './__tests__/problems/2332_the-latest-time-to-catch-a-bus/2332_the-latest-time-to-catch-a-bus_response.txt',
                        'utf-8',
                ),
        );
});

describe('2332_the-latest-time-to-catch-a-bus', () => {
        test('solution template', () => {
                ...
                expect(solution).toMatchSnapshot('2332_the-latest-time-to-catch-a-bus');
        });
});
