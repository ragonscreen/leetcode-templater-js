...
beforeAll(async () => {
        SOURCE = JSON.parse(
                await readFile(
                        './__tests__/problems/0441_arranging-coins/0441_arranging-coins_response.txt',
                        'utf-8',
                ),
        );
});

describe('0441_arranging-coins', () => {
        test('solution template', () => {
                ...
                expect(solution).toMatchSnapshot('0441_arranging-coins');
        });
});
