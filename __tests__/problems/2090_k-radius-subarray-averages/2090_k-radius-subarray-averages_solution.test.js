...
beforeAll(async () => {
        SOURCE = JSON.parse(
                await readFile(
                        './__tests__/problems/2090_k-radius-subarray-averages/2090_k-radius-subarray-averages_response.txt',
                        'utf-8',
                ),
        );
});

describe('2090_k-radius-subarray-averages', () => {
        test('solution template', () => {
                ...
                expect(solution).toMatchSnapshot('2090_k-radius-subarray-averages');
        });
});
