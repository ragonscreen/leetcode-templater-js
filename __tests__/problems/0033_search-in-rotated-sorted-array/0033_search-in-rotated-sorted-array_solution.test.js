...
beforeAll(async () => {
        SOURCE = JSON.parse(
                await readFile(
                        './__tests__/problems/0033_search-in-rotated-sorted-array/0033_search-in-rotated-sorted-array_response.txt',
                        'utf-8',
                ),
        );
});

describe('0033_search-in-rotated-sorted-array', () => {
        test('solution template', () => {
                ...
                expect(solution).toMatchSnapshot('0033_search-in-rotated-sorted-array');
        });
});
