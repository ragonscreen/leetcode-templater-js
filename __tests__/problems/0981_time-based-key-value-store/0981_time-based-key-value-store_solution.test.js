...
beforeAll(async () => {
        SOURCE = JSON.parse(
                await readFile(
                        './__tests__/problems/0981_time-based-key-value-store/0981_time-based-key-value-store_response.txt',
                        'utf-8',
                ),
        );
});

describe('0981_time-based-key-value-store', () => {
        test('solution template', () => {
                ...
                expect(solution).toMatchSnapshot('0981_time-based-key-value-store');
        });
});
