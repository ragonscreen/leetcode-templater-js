...
beforeAll(async () => {
        SOURCE = JSON.parse(
                await readFile(
                        './__tests__/problems/0219_contains-duplicate-ii/0219_contains-duplicate-ii_response.txt',
                        'utf-8',
                ),
        );
});

describe('0219_contains-duplicate-ii', () => {
        test('solution template', () => {
                ...
                expect(solution).toMatchSnapshot('0219_contains-duplicate-ii');
        });
});
