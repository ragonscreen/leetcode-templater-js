...
beforeAll(async () => {
        SOURCE = JSON.parse(
                await readFile(
                        './__tests__/problems/2161_partition-array-according-to-given-pivot/2161_partition-array-according-to-given-pivot_response.txt',
                        'utf-8',
                ),
        );
});

describe('2161_partition-array-according-to-given-pivot', () => {
        test('solution template', () => {
                ...
                expect(solution).toMatchSnapshot('2161_partition-array-according-to-given-pivot');
        });
});
