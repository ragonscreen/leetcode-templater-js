...
beforeAll(async () => {
        SOURCE = JSON.parse(
                await readFile(
                        './__tests__/problems/2226_maximum-candies-allocated-to-k-children/2226_maximum-candies-allocated-to-k-children_response.txt',
                        'utf-8',
                ),
        );
});

describe('2226_maximum-candies-allocated-to-k-children', () => {
        test('solution template', () => {
                ...
                expect(solution).toMatchSnapshot('2226_maximum-candies-allocated-to-k-children');
        });
});
