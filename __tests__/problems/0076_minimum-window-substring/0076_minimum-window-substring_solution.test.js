...
beforeAll(async () => {
        SOURCE = JSON.parse(
                await readFile(
                        './__tests__/problems/0076_minimum-window-substring/0076_minimum-window-substring_response.txt',
                        'utf-8',
                ),
        );
});

describe('0076_minimum-window-substring', () => {
        test('solution template', () => {
                ...
                expect(solution).toMatchSnapshot('0076_minimum-window-substring');
        });
});
