...
beforeAll(async () => {
        SOURCE = JSON.parse(
                await readFile(
                        './__tests__/problems/0023_merge-k-sorted-lists/0023_merge-k-sorted-lists_response.txt',
                        'utf-8',
                ),
        );
});

describe('0023_merge-k-sorted-lists', () => {
        test('solution template', () => {
                ...
                expect(solution).toMatchSnapshot('0023_merge-k-sorted-lists');
        });
});
