...
beforeAll(async () => {
        SOURCE = JSON.parse(
                await readFile(
                        './__tests__/problems/2423_remove-letter-to-equalize-frequency/2423_remove-letter-to-equalize-frequency_response.txt',
                        'utf-8',
                ),
        );
});

describe('2423_remove-letter-to-equalize-frequency', () => {
        test('solution template', () => {
                ...
                expect(solution).toMatchSnapshot('2423_remove-letter-to-equalize-frequency');
        });
});
