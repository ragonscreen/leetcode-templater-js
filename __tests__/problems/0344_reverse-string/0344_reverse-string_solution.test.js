...
beforeAll(async () => {
        SOURCE = JSON.parse(
                await readFile(
                        './__tests__/problems/0344_reverse-string/0344_reverse-string_response.txt',
                        'utf-8',
                ),
        );
});

describe('0344_reverse-string', () => {
        test('solution template', () => {
                ...
                expect(solution).toMatchSnapshot('0344_reverse-string');
        });
});
