...
beforeAll(async () => {
        SOURCE = JSON.parse(
                await readFile(
                        './__tests__/problems/0424_longest-repeating-character-replacement/0424_longest-repeating-character-replacement_response.txt',
                        'utf-8',
                ),
        );
});

describe('0424_longest-repeating-character-replacement', () => {
        test('solution template', () => {
                ...
                expect(solution).toMatchSnapshot('0424_longest-repeating-character-replacement');
        });
});
