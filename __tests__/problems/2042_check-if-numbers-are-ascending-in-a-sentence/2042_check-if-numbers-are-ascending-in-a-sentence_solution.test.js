...
beforeAll(async () => {
        SOURCE = JSON.parse(
                await readFile(
                        './__tests__/problems/2042_check-if-numbers-are-ascending-in-a-sentence/2042_check-if-numbers-are-ascending-in-a-sentence_response.txt',
                        'utf-8',
                ),
        );
});

describe('2042_check-if-numbers-are-ascending-in-a-sentence', () => {
        test('solution template', () => {
                ...
                expect(solution).toMatchSnapshot('2042_check-if-numbers-are-ascending-in-a-sentence');
        });
});
