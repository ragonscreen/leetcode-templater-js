...
beforeAll(async () => {
        SOURCE = JSON.parse(
                await readFile(
                        './__tests__/problems/0060_permutation-sequence/0060_permutation-sequence_response.txt',
                        'utf-8',
                ),
        );
});

describe('0060_permutation-sequence', () => {
        test('solution template', () => {
                ...
                expect(solution).toMatchSnapshot('0060_permutation-sequence');
        });
});
