...
beforeAll(async () => {
        SOURCE = JSON.parse(
                await readFile(
                        './__tests__/problems/0643_maximum-average-subarray-i/0643_maximum-average-subarray-i_response.txt',
                        'utf-8',
                ),
        );
});

describe('0643_maximum-average-subarray-i', () => {
        test('solution template', () => {
                ...
                expect(solution).toMatchSnapshot('0643_maximum-average-subarray-i');
        });
});
