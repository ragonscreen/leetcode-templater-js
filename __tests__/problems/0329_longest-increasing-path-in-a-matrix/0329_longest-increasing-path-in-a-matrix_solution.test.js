...
beforeAll(async () => {
        SOURCE = JSON.parse(
                await readFile(
                        './__tests__/problems/0329_longest-increasing-path-in-a-matrix/0329_longest-increasing-path-in-a-matrix_response.txt',
                        'utf-8',
                ),
        );
});

describe('0329_longest-increasing-path-in-a-matrix', () => {
        test('solution template', () => {
                ...
                expect(solution).toMatchSnapshot('0329_longest-increasing-path-in-a-matrix');
        });
});
