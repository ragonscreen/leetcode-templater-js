...
beforeAll(async () => {
        SOURCE = JSON.parse(
                await readFile(
                        './__tests__/problems/0622_design-circular-queue/0622_design-circular-queue_response.txt',
                        'utf-8',
                ),
        );
});

describe('0622_design-circular-queue', () => {
        test('solution template', () => {
                ...
                expect(solution).toMatchSnapshot('0622_design-circular-queue');
        });
});
