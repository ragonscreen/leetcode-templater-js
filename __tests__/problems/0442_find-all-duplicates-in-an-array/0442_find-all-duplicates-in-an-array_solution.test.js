...
beforeAll(async () => {
        SOURCE = JSON.parse(
                await readFile(
                        './__tests__/problems/0442_find-all-duplicates-in-an-array/0442_find-all-duplicates-in-an-array_response.txt',
                        'utf-8',
                ),
        );
});

describe('0442_find-all-duplicates-in-an-array', () => {
        test('solution template', () => {
                ...
                expect(solution).toMatchSnapshot('0442_find-all-duplicates-in-an-array');
        });
});
