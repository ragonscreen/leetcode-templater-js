...
beforeAll(async () => {
        SOURCE = JSON.parse(
                await readFile(
                        './__tests__/problems/0791_custom-sort-string/0791_custom-sort-string_response.txt',
                        'utf-8',
                ),
        );
});

describe('0791_custom-sort-string', () => {
        test('solution template', () => {
                ...
                expect(solution).toMatchSnapshot('0791_custom-sort-string');
        });
});
