...
beforeAll(async () => {
        SOURCE = JSON.parse(
                await readFile(
                        './__tests__/problems/1970_last-day-where-you-can-still-cross/1970_last-day-where-you-can-still-cross_response.txt',
                        'utf-8',
                ),
        );
});

describe('1970_last-day-where-you-can-still-cross', () => {
        test('solution template', () => {
                ...
                expect(solution).toMatchSnapshot('1970_last-day-where-you-can-still-cross');
        });
});
