...
beforeAll(async () => {
        SOURCE = JSON.parse(
                await readFile(
                        './__tests__/problems/1209_remove-all-adjacent-duplicates-in-string-ii/1209_remove-all-adjacent-duplicates-in-string-ii_response.txt',
                        'utf-8',
                ),
        );
});

describe('1209_remove-all-adjacent-duplicates-in-string-ii', () => {
        test('solution template', () => {
                ...
                expect(solution).toMatchSnapshot('1209_remove-all-adjacent-duplicates-in-string-ii');
        });
});
