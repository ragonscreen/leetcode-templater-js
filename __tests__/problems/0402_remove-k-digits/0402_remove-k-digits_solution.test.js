...
beforeAll(async () => {
        SOURCE = JSON.parse(
                await readFile(
                        './__tests__/problems/0402_remove-k-digits/0402_remove-k-digits_response.txt',
                        'utf-8',
                ),
        );
});

describe('0402_remove-k-digits', () => {
        test('solution template', () => {
                ...
                expect(solution).toMatchSnapshot('0402_remove-k-digits');
        });
});
