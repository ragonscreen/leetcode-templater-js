...
beforeAll(async () => {
        SOURCE = JSON.parse(
                await readFile(
                        './__tests__/problems/0179_largest-number/0179_largest-number_response.txt',
                        'utf-8',
                ),
        );
});

describe('0179_largest-number', () => {
        test('solution template', () => {
                ...
                expect(solution).toMatchSnapshot('0179_largest-number');
        });
});
