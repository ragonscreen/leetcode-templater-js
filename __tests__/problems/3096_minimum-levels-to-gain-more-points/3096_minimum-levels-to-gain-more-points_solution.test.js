...
beforeAll(async () => {
        SOURCE = JSON.parse(
                await readFile(
                        './__tests__/problems/3096_minimum-levels-to-gain-more-points/3096_minimum-levels-to-gain-more-points_response.txt',
                        'utf-8',
                ),
        );
});

describe('3096_minimum-levels-to-gain-more-points', () => {
        test('solution template', () => {
                ...
                expect(solution).toMatchSnapshot('3096_minimum-levels-to-gain-more-points');
        });
});
