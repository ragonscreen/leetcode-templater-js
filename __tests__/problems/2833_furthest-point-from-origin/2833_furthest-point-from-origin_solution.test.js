...
beforeAll(async () => {
        SOURCE = JSON.parse(
                await readFile(
                        './__tests__/problems/2833_furthest-point-from-origin/2833_furthest-point-from-origin_response.txt',
                        'utf-8',
                ),
        );
});

describe('2833_furthest-point-from-origin', () => {
        test('solution template', () => {
                ...
                expect(solution).toMatchSnapshot('2833_furthest-point-from-origin');
        });
});
