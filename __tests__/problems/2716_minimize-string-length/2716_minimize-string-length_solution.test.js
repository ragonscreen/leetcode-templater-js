...
beforeAll(async () => {
        SOURCE = JSON.parse(
                await readFile(
                        './__tests__/problems/2716_minimize-string-length/2716_minimize-string-length_response.txt',
                        'utf-8',
                ),
        );
});

describe('2716_minimize-string-length', () => {
        test('solution template', () => {
                ...
                expect(solution).toMatchSnapshot('2716_minimize-string-length');
        });
});
