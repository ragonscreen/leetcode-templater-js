...
beforeAll(async () => {
        SOURCE = JSON.parse(
                await readFile(
                        './__tests__/problems/0153_find-minimum-in-rotated-sorted-array/0153_find-minimum-in-rotated-sorted-array_response.txt',
                        'utf-8',
                ),
        );
});

describe('0153_find-minimum-in-rotated-sorted-array', () => {
        test('solution template', () => {
                ...
                expect(solution).toMatchSnapshot('0153_find-minimum-in-rotated-sorted-array');
        });
});
