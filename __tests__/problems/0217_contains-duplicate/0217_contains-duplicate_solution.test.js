...
beforeAll(async () => {
        SOURCE = JSON.parse(
                await readFile(
                        './__tests__/problems/0217_contains-duplicate/0217_contains-duplicate_response.txt',
                        'utf-8',
                ),
        );
});

describe('0217_contains-duplicate', () => {
        test('solution template', () => {
                ...
                expect(solution).toMatchSnapshot('0217_contains-duplicate');
        });
});
