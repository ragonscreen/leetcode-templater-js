...
beforeAll(async () => {
        SOURCE = JSON.parse(
                await readFile(
                        './__tests__/problems/2195_append-k-integers-with-minimal-sum/2195_append-k-integers-with-minimal-sum_response.txt',
                        'utf-8',
                ),
        );
});

describe('2195_append-k-integers-with-minimal-sum', () => {
        test('solution template', () => {
                ...
                expect(solution).toMatchSnapshot('2195_append-k-integers-with-minimal-sum');
        });
});
