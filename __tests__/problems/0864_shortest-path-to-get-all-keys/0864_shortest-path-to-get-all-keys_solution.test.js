...
beforeAll(async () => {
        SOURCE = JSON.parse(
                await readFile(
                        './__tests__/problems/0864_shortest-path-to-get-all-keys/0864_shortest-path-to-get-all-keys_response.txt',
                        'utf-8',
                ),
        );
});

describe('0864_shortest-path-to-get-all-keys', () => {
        test('solution template', () => {
                ...
                expect(solution).toMatchSnapshot('0864_shortest-path-to-get-all-keys');
        });
});
