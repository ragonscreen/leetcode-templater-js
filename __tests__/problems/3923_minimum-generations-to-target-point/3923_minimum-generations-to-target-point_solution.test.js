...
beforeAll(async () => {
        SOURCE = JSON.parse(
                await readFile(
                        './__tests__/problems/3923_minimum-generations-to-target-point/3923_minimum-generations-to-target-point_response.txt',
                        'utf-8',
                ),
        );
});

describe('3923_minimum-generations-to-target-point', () => {
        test('solution template', () => {
                ...
                expect(solution).toMatchSnapshot('3923_minimum-generations-to-target-point');
        });
});
