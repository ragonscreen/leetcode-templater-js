...
beforeAll(async () => {
        SOURCE = JSON.parse(
                await readFile(
                        './__tests__/problems/0487_max-consecutive-ones-ii/0487_max-consecutive-ones-ii_response.txt',
                        'utf-8',
                ),
        );
});

describe('0487_max-consecutive-ones-ii', () => {
        test('solution template', () => {
                ...
                expect(solution).toMatchSnapshot('0487_max-consecutive-ones-ii');
        });
});
