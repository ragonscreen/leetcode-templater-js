...
beforeAll(async () => {
        SOURCE = JSON.parse(
                await readFile(
                        './__tests__/problems/0084_largest-rectangle-in-histogram/0084_largest-rectangle-in-histogram_response.txt',
                        'utf-8',
                ),
        );
});

describe('0084_largest-rectangle-in-histogram', () => {
        test('solution template', () => {
                ...
                expect(solution).toMatchSnapshot('0084_largest-rectangle-in-histogram');
        });
});
