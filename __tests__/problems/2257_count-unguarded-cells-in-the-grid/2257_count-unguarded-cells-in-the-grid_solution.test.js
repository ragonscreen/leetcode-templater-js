...
beforeAll(async () => {
        SOURCE = JSON.parse(
                await readFile(
                        './__tests__/problems/2257_count-unguarded-cells-in-the-grid/2257_count-unguarded-cells-in-the-grid_response.txt',
                        'utf-8',
                ),
        );
});

describe('2257_count-unguarded-cells-in-the-grid', () => {
        test('solution template', () => {
                ...
                expect(solution).toMatchSnapshot('2257_count-unguarded-cells-in-the-grid');
        });
});
