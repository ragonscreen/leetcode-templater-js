...
beforeAll(async () => {
        SOURCE = JSON.parse(
                await readFile(
                        './__tests__/problems/2121_intervals-between-identical-elements/2121_intervals-between-identical-elements_response.txt',
                        'utf-8',
                ),
        );
});

describe('2121_intervals-between-identical-elements', () => {
        test('solution template', () => {
                ...
                expect(solution).toMatchSnapshot('2121_intervals-between-identical-elements');
        });
});
