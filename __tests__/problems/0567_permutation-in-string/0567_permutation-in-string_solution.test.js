...
beforeAll(async () => {
        SOURCE = JSON.parse(
                await readFile(
                        './__tests__/problems/0567_permutation-in-string/0567_permutation-in-string_response.txt',
                        'utf-8',
                ),
        );
});

describe('0567_permutation-in-string', () => {
        test('solution template', () => {
                ...
                expect(solution).toMatchSnapshot('0567_permutation-in-string');
        });
});
