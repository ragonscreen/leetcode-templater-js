...
beforeAll(async () => {
        SOURCE = JSON.parse(
                await readFile(
                        './__tests__/problems/0268_missing-number/0268_missing-number_response.txt',
                        'utf-8',
                ),
        );
});

describe('0268_missing-number', () => {
        test('solution template', () => {
                ...
                expect(solution).toMatchSnapshot('0268_missing-number');
        });
});
