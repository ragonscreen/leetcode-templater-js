...
beforeAll(async () => {
        SOURCE = JSON.parse(
                await readFile(
                        './__tests__/problems/0232_implement-queue-using-stacks/0232_implement-queue-using-stacks_response.txt',
                        'utf-8',
                ),
        );
});

describe('0232_implement-queue-using-stacks', () => {
        test('solution template', () => {
                ...
                expect(solution).toMatchSnapshot('0232_implement-queue-using-stacks');
        });
});
