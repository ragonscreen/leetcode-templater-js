...
beforeAll(async () => {
        SOURCE = JSON.parse(
                await readFile(
                        './__tests__/problems/1658_minimum-operations-to-reduce-x-to-zero/1658_minimum-operations-to-reduce-x-to-zero_response.txt',
                        'utf-8',
                ),
        );
});

describe('1658_minimum-operations-to-reduce-x-to-zero', () => {
        test('test template', () => {
                ...
                expect(test).toMatchSnapshot('1658_minimum-operations-to-reduce-x-to-zero');
        });
});
